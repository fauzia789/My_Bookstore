...
function Navbar() {
 ...
 <div className="hidden md:flex flex gap-4">   {links.map((items,i)=> ( <Link to =
 {items.link}
...
<button className="text-white text-2xl block md:hidden hover:text-zinc-400" onClick={()=> MobileNav === "hidden" ? setMobileNav("block") : setMobileNav("hidden")}><FaGripLines /></button>
    </div>
    </nav>
    <div className={` ${MobileNav} bg-zinc-800 h-screen absolute top-0 left-0 w-full z-40 flex flex-col items-center justify-center`}>
    {links.map((items,i)=> ( <Link to =
 {items.link}
  className={` ${MobileNav} hover:text-blue-500 text-white text-4xl mb-8 font-semibold transition-all duration-500`} key={i}
  onClick={()=> MobileNav === "hidden" ? setMobileNav("block") : setMobileNav("hidden")}
  >{items.title}{" "}</Link>))}
  
 
    <Link to="/LogIn" className={` ${MobileNav} px-8 py-2 mb-8 text-3xl font-semibold border border-blue-500 rounded text-white hover:bg-white hover:text-zinc-800 transition-all duration-300`}>Login</Link>
    <Link to="/SignUp" className={` ${MobileNav} px-8 py-2  mb-8 text-3xl font-semibold  bg-blue-500 rounded  text-white hover:text-zinc-800 transition-all duration-300`}>SignUp</Link>
